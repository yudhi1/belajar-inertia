...
<template>
    ...
    <AppLayout>
        ...
        <h1 class="mb-6 text-2xl font-bold tracking-tight">New Project</h1>

        <Card class="max-w-2xl p-6">
            <form class="space-y-5" @submit.prevent="submit">
                <div>
                    <Label for="name">Name</Label>
                    <Input id="name" v-model="form.name" placeholder="Website redesign" />
                    <p v-if="form.errors.name" class="mt-1 text-xs text-destructive">{{ form.errors.name }}</p>
                </div>
                <div>
                    <Label for="description">Description</Label>
                    <Textarea id="description" v-model="form.description" placeholder="What is this project about?" />
